...
Analyzes the relationship between dimension product and average kernel duration
"""
...
def create_visualizations(X, y, results, output_dir='.'):
    ...
    fig.suptitle('Linear Regression Analysis: dim_product vs avg_duration_us', fontsize=16, fontweight='bold')
    ...
    ax1.set_xlabel('dim_product (Dimension Product)')
...
def print_detailed_results(results):
    ...
    print(f"\nModel Equation:")
    print(f"avg_duration_us = {results['slope']:.6f} × dim_product + {results['intercept']:.6f}")
    ...
    if results['p_value'] < 0.05:
        ...
        print(f"  • For every unit increase in dim_product, avg_duration_us {direction} by {abs(results['slope']):.6f} microseconds")
...
def save_results_to_csv(X, y, results, output_file):
    ...
    with open(output_file, 'w') as f:
        ...
        f.write(f"# Model: avg_duration_us = {results['slope']:.6f} * dim_product + {results['intercept']:.6f}\n")
