#!/usr/bin/env python3
"""
Linear Regression Analysis: dim_0 vs avg_duration_us
Analyzes the relationship between dimension 0 size and average kernel duration
"""
...
def load_and_prepare_data(csv_file):
    ...
    Returns:
        tuple: (X, y, df) where X is dim_0, y is avg_duration_us, df is the dataframe
    """
    ...
    if 'dim_0' not in df.columns or 'avg_duration_us' not in df.columns:
        raise ValueError("Required columns 'dim_0' and 'avg_duration_us' not found in CSV")
    
    # Remove any rows with NaN values in the columns of interest
    df_clean = df.dropna(subset=['dim_0', 'avg_duration_us'])
    print(f"After removing NaN values: {len(df_clean)} rows (removed {len(df) - len(df_clean)} rows)")
    
    # Extract X and y variables
    X = df_clean['dim_0'].values.reshape(-1, 1)  # reshape for sklearn
    y = df_clean['avg_duration_us'].values
    
    print(f"X (dim_0) range: [{X.min():.0f}, {X.max():.0f}]")
...
def perform_linear_regression(X, y):
    ...
    Args:
        X (array): Independent variable (dim_0)
...
def create_visualizations(X, y, results, output_dir='.'):
    ...
    fig.suptitle('Linear Regression Analysis: dim_0 vs avg_duration_us', fontsize=16, fontweight='bold')
    ...
    ax1.set_xlabel('dim_0 (Dimension Size)')
...
def print_detailed_results(results):
    ...
    print(f"\nModel Equation:")
    print(f"avg_duration_us = {results['slope']:.6f} × dim_0 + {results['intercept']:.6f}")
    ...
    if results['p_value'] < 0.05:
        ...
        print(f"  • There is a statistically significant relationship between dim_0 and avg_duration_us")
        print(f"  • For every unit increase in dim_0, avg_duration_us {direction} by {abs(results['slope']):.6f} microseconds")
        print(f"  • The model explains {results['r_squared']*100:.1f}% of the variance in avg_duration_us")
    else:
        print(f"  • No statistically significant relationship found between dim_0 and avg_duration_us")
...
def save_results_to_csv(X, y, results, output_file):
    ...
    with open(output_file, 'w') as f:
        ...
        f.write(f"# Model: avg_duration_us = {results['slope']:.6f} * dim_0 + {results['intercept']:.6f}\n")
