...
def generate_uniform_test_shapes(
    n_shapes: int,
    max_numel: int,
    dim_probs: tuple = (0.33, 0.33, 0.33),
    seed: int = 42,
) -> list:
    """
    Generate test shapes with UNIFORM distribution of tensor sizes.
    
    This replaces the log-uniform distribution used in dataset_generation.py
    with a true uniform distribution across the numel range.
    
    Args:
        n_shapes: Number of shapes to generate
        max_numel: Maximum number of elements
        dim_probs: Probabilities for (1D, 2D, 3D) shapes
        seed: Random seed
    
    Returns:
        List of (d0, d1, d2) tuples
    """
    ...
    attempts = 0
    max_attempts = n_shapes * 100
    
    while len(shapes) < n_shapes and attempts < max_attempts:
        attempts += 1
        ...
        # UNIFORM sampling of numel (this is the key change!)
        if dim == 1:
            numel = rng.randint(1, max_numel + 1)
            shape = (numel, 1, 1)
        
        elif dim == 2:
            # For 2D: sample numel uniformly, then factor
            numel = rng.randint(2, max_numel + 1)  # at least 2 for 2D
            # Find a random factorization
            sqrt_n = int(np.sqrt(numel))
            # Sample d0 from valid range
            max_d0 = min(numel, max_numel)
            d0 = rng.randint(1, max_d0 + 1)
            d1 = numel // d0
            
            # If d1 is too large, resample
            if d1 * d0 > max_numel or d1 < 1:
                # Just make a valid 2D shape within constraints
                d0 = rng.randint(1, int(np.sqrt(max_numel)) + 1)
                d1 = min(numel // d0, max_numel // d0)
                d1 = max(1, d1)
            
            # Random swap for variety
            if rng.random() < 0.5:
                d0, d1 = d1, d0
            ...
            # For 3D: sample numel uniformly, then create 3D factorization
            numel = rng.randint(8, max_numel + 1)  # at least 8 for 3D (2*2*2)
            
            # Create 3D shape by sampling dimensions
            cbrt = int(numel ** (1/3))
            max_dim = max(2, min(cbrt * 2, int(np.sqrt(max_numel))))
            
            d0 = rng.randint(2, max_dim + 1)
            d1 = rng.randint(2, max(2, max_numel // d0) + 1)
            max_d2 = max(2, max_numel // (d0 * d1))
            d2 = rng.randint(2, max_d2 + 1)
            
            # Ensure we don't exceed max_numel
            if d0 * d1 * d2 > max_numel:
                d2 = max(2, max_numel // (d0 * d1))
            
            shape = (d0, d1, d2)
        
        # Validate and add shape
        d0, d1, d2 = shape
        actual_numel = d0 * d1 * d2
        
        if actual_numel > max_numel or actual_numel < 1:
            continue
        
        # Check for valid shape structure
        if d2 > 1 and (d0 == 1 or d1 == 1):
            continue  # Invalid 3D shape
...
def run_pipeline(
    ...
    print(f"Generating {n_test_shapes} test shapes with UNIFORM distribution (seed={test_seed})...")
    test_shapes = generate_uniform_test_shapes(
        n_shapes=n_test_shapes,
        max_numel=max_numel,
        dim_probs=(0.33, 0.33, 0.33),
        seed=test_seed,
    )
    
    print(f"Generated {len(test_shapes)} unique test shapes with uniform size distribution")
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="mul",
        help="Operation name (default: mul)"
    )
    parser.add_argument(
        "--dtype",
        type=str,
