"""
Profile elementwise multiply operation across a generated dataset of shapes.

This script generates a diverse set of tensor shapes using the dataset_generation
module and profiles the elementwise multiply operation on TPU for each shape.
"""
...
from dataset_generation import generate_shapes_for_latency
...
def create_profiling_configuration(
    ...
    storage_file = os.path.join(output_dir, f"mul_dataset_{timestamp}.csv")
    metadata_file = os.path.join(output_dir, f"mul_metadata_{timestamp}.csv")
    ...
    return {
        ...
        "random_seed": seed,
        "repo_version": "v0.0.1",
        "comment": f"elementwise_mul_dataset n={n_shapes} max_numel={max_numel} seed={seed} {comment}",
    }


def profile_elementwise_mul_dataset(
    output_dir: str = "./mul_profiling_results",
    n_shapes: int = 2000,
    ...
    """
    Generate shapes and profile elementwise multiply operation.
    ...
    Args:
        ...
        dim_probs: Probability distribution for 1D, 2D, 3D shapes.
        boundary_frac: Fraction of shapes focused on boundary conditions.
        perturb_range: Perturbation range for boundary-focused shapes.
    ...
    shapes = generate_shapes_for_latency(
        n_shapes=n_shapes,
        max_numel=max_numel,
        dim_probs=dim_probs,
    ...
    for i, (d0, d1, d2) in enumerate(shapes):
        ...
        # Create kernel wrapper for elementwise multiply (both inputs same shape)
        ...
        profiler_name = f"mul_{shape_str}_{i:05d}"
...
def main():
    ...
    default_output_dir = f"./mul_dataset/mul_profiling_results_{timestamp}"
    default_n_shapes = 2000
    default_max_numel = 16*1024*1024
    default_seed = 0
    default_dim_probs = (0.33, 0.33, 0.33)
    ...
    parser = argparse.ArgumentParser(
        description="Profile elementwise multiply operation across a dataset of shapes"
    )
    parser.add_argument(
        "--output-dir", "-o",
        type=str,
    ...
    parser.add_argument(
        ...
        help=f"Probability distribution for 1D, 2D, 3D shapes (default: {default_dim_probs})"
    )
    parser.add_argument(
        "--boundary-frac",
        type=float,
        default=default_boundary_frac,
        help=f"Fraction of shapes focused on boundary conditions (default: {default_boundary_frac})"
    )
    parser.add_argument(
        "--perturb-range",
        type=int,
        default=default_perturb_range,
        help=f"Perturbation range for boundary-focused shapes (default: {default_perturb_range})"
    )
    parser.add_argument(
        "--dtype",
        type=str,
    ...
    profile_elementwise_mul_dataset(
