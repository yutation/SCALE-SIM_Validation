"""
Profile ReLU operation across a generated dataset of shapes.

This script generates a diverse set of tensor shapes using the dataset_generation
module and profiles the ReLU activation operation on TPU for each shape.
"""
...
def create_profiling_configuration(
    ...
    storage_file = os.path.join(output_dir, f"relu_dataset_{timestamp}.csv")
    metadata_file = os.path.join(output_dir, f"relu_metadata_{timestamp}.csv")
    ...
    return {
        ...
        "random_seed": seed,
        "repo_version": "v0.0.1",
        "comment": f"relu_dataset n={n_shapes} max_numel={max_numel} seed={seed} {comment}",
    }


def profile_elementwise_relu_dataset(
    output_dir: str = "./relu_profiling_results",
    n_shapes: int = 2000,
    ...
    """
    Generate shapes and profile ReLU activation operation.
    ...
    for i, (d0, d1, d2) in enumerate(shapes):
        ...
        # Create kernel wrapper for ReLU (single input)
        ...
        profiler_name = f"relu_{shape_str}_{i:05d}"
...
def main():
    ...
    default_output_dir = f"./relu_dataset/relu_profiling_results_{timestamp}"
    ...
    parser = argparse.ArgumentParser(
        description="Profile ReLU activation operation across a dataset of shapes"
    )
    parser.add_argument(
        "--output-dir", "-o",
        type=str,
    ...
    profile_elementwise_relu_dataset(
