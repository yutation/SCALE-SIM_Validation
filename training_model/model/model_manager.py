"""
Model training, saving, loading, and inference utilities.
...
from train import load_and_prepare, make_features, split_by_log_bins
...
def train_and_save(
    ...
    """
    Train model on dataset and save to disk.
    ...
    """
    print(f"\n{'='*60}")
    print(f"Training model on: {csv_path}")
    print(f"{'='*60}\n")
...
def predict_latency(
    ...
    """
    Predict latencies for a list of shapes.
    ...
    df = pd.DataFrame({
        ...
        "input_dim_2": [s[2] for s in shapes],
    })
    df["size"] = df["input_dim_0"] * df["input_dim_1"] * df["input_dim_2"]
...
def predict_from_saved_model(
    ...
    """
    Load model and predict latencies.
...
if __name__ == "__main__":
    ...
    csv_path = script_dir / "add_dataset_20260202_170757.csv"
    model_path = script_dir / "trained_models" / "add_model.pkl"
    ...
    if csv_path.exists():
        ...
        # Test prediction
        test_shapes = [(100, 1, 1), (256, 256, 1), (64, 64, 64)]
