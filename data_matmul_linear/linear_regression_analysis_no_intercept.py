...
def perform_linear_regression_no_intercept(X, y):
    """
    Perform linear regression with intercept=0 and return the model and metrics
    """
...
def print_results(slope, intercept, r2, mse, rmse):
    ...
    """
    print("\n" + "="*70)
    print("LINEAR REGRESSION RESULTS (NO INTERCEPT)")
    print("="*70)
    ...
    print(f"- The regression line passes through the origin (0,0)")
    ...
    print("This may result in a different R² value compared to the model with intercept.")

def generate_output_filenames(input_file_path):
    """
    Generate output filenames based on input filename for no-intercept analysis
    """
    # Get the base name without extension
    base_name = os.path.splitext(os.path.basename(input_file_path))[0]
    results_file = f"{base_name}_linear_regression_no_intercept_results.txt"
    plot_file = f"{base_name}_linear_regression_no_intercept_analysis.png"
    return results_file, plot_file

def create_visualizations(df, X, y, y_pred, slope, intercept, plot_filename):
    """
    Create comprehensive visualizations of the linear regression with no intercept
    """
    ...
    fig.suptitle('Linear Regression Analysis (No Intercept): total_cycles vs fusion_avg', fontsize=16, fontweight='bold')
    
    # 1. Scatter plot with regression line
    axes[0, 0].scatter(X, y, alpha=0.6, s=30, label='Data points')
    axes[0, 0].plot(X, y_pred, color='red', linewidth=2, label=f'Regression line: y = {slope:.6f}x')
    axes[0, 0].axhline(y=0, color='black', linestyle='-', alpha=0.3, linewidth=1)
    axes[0, 0].axvline(x=0, color='black', linestyle='-', alpha=0.3, linewidth=1)
    axes[0, 0].set_xlabel('Total Cycles')
    axes[0, 0].set_ylabel('Fusion Average')
    axes[0, 0].set_title('Linear Regression Fit (No Intercept)')
    axes[0, 0].legend()
...
def main():
    """
    Main function to run the complete analysis with no intercept
    """
    # File path - now in the same directory
    csv_file = "kernel_report_updated6.csv"
    ...
    try:
        ...
        model, y_pred, slope, intercept, r2, mse, rmse = perform_linear_regression_no_intercept(X, y)
        ...
        with open(results_file, 'w') as f:
            f.write("COMPREHENSIVE LINEAR REGRESSION RESULTS (NO INTERCEPT)\n")
            f.write("=" * 60 + "\n")
            ...
            f.write(f"\nNote: This model forces the regression line through the origin (0,0)\n")
            f.write(f"This may result in a different R² value compared to the model with intercept.\n")
            f.write(f"\nExample Predictions:\n")
